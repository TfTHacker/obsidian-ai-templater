...
export const initializeTemplaterInternalModule = async (plugin: AitPlugin): Promise<InternalModuleAit> => {
  return new Promise((resolve, reject) => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (templater) {
        ...
        templater.templater.functions_generator.internal_functions.modules_array.push(internal_module);
        internal_module
          .init()
          .then(() => {
          ...
          .catch((error: unknown) => {
            if (error instanceof Error) {
              console.error('Error initializing internal module:', error.message);
              reject(error);
            } else {
      ...
      enablePlugin(oldMethod) {
        return async function (pluginId) {
          if (pluginId === 'templater-obsidian') {
            ...
            setTimeout(async () => {
              if (window.ait?.plugin) window.ait.plugin.internalModuleAit = await initializeTemplaterInternalModule(window.ait.plugin);
            }, 1000);
          }
